...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    fprintf(stdout,"arguments manquants: nombre de threads, puis nombre de yield\n");
    return -1;
  }
  ...
  fprintf(stdout,"%d yield avec plein de threads dans join: %ld us\n", nbyield, us);

  fprintf(stdout,"%ld threads créés et détruits\n", nbth);
  return 0;
}
